    ...
    public class Readers
    {
        ...
        MethodReference GenerateReader(TypeReference variableReference, ref bool WeavingFailed)
        {
            ...
            if (variableReference.IsArray)
            {
                ...
                return GenerateReadCollection(variableReference, variableReference.GetElementType(),
                    nameof(NetworkReaderExtensions.ReadArray), ref WeavingFailed);
            }
        ...
        MethodDefinition GenerateArraySegmentReadFunc(TypeReference variable, ref bool WeavingFailed)
        {
            ...
            worker.Emit(OpCodes.Newobj,
                weaverTypes.ArraySegmentConstructorReference.MakeHostInstanceGeneric(assembly.MainModule,
                    genericInstance));
            worker.Emit(OpCodes.Ret);
            return readerFunc;
        }
        ...
        MethodDefinition GenerateReadCollection(TypeReference variable, TypeReference elementType,
            string readerFunction, ref bool WeavingFailed)
        {
            MethodDefinition readerFunc = GenerateReaderFunction(variable);
            ...
            MethodReference listReader =
                Resolvers.ResolveMethod(readerExtensions, assembly, Log, readerFunction, ref WeavingFailed);
        ...
        void CreateNew(TypeReference variable, ILProcessor worker, TypeDefinition td, ref bool WeavingFailed)
        {
            ...
            else if (td.IsDerivedFrom<UnityEngine.ScriptableObject>())
            {
                GenericInstanceMethod genericInstanceMethod =
                    new GenericInstanceMethod(weaverTypes.ScriptableObjectCreateInstanceMethod);
        ...
        internal void InitializeReaders(ILProcessor worker)
        {
            ...
            foreach (KeyValuePair<TypeReference, MethodReference> kvp in readFuncs)
            {
                ...
                MethodReference funcConstructorInstance =
                    funcConstructorRef.MakeHostInstanceGeneric(assembly.MainModule, funcGenericInstance);
