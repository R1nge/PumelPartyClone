...
namespace Mirror.Editor.Weaver.Processors
{
    ...
    public class SyncVarAttributeProcessor
    {
        ...
        public SyncVarAttributeProcessor(AssemblyDefinition assembly, WeaverTypes weaverTypes,
            SyncVarAccessLists syncVarAccessLists, Logger Log)
        {
        ...
        public FieldDefinition CreateNewActionFieldDefinitionFromHookMethod(FieldDefinition syncVarField)
        {
            ...
            GenericInstanceType syncVarHookActionDelegateType =
                actionRef.MakeGenericInstanceType(syncVarField.FieldType, syncVarField.FieldType);
            string syncVarHookDelegateFieldName = $"_Mirror_SyncVarHookDelegate_{syncVarField.Name}";
            return new FieldDefinition(syncVarHookDelegateFieldName, FieldAttributes.Public,
                syncVarHookActionDelegateType);
        }
            ...
            MethodDefinition hookMethod)
        {
            ...
            {
                hookMethodReference = hookMethod.MakeHostInstanceGeneric(hookMethod.Module,
        ...
        public void GenerateSyncVarHookDelegateInitializer(ILProcessor worker, FieldDefinition syncVar,
            FieldDefinition hookDelegate, MethodDefinition hookMethod)
        {
            // push this
            worker.Emit(OpCodes.Ldarg_0);
            ...
            ref bool WeavingFailed)
        {
            ...
            List<MethodDefinition> methodsWith2Param =
                new List<MethodDefinition>(methods.Where(m => m.Parameters.Count == 2));
        ...
        public MethodDefinition GenerateSyncVarGetter(FieldDefinition fd, string originalName,
            FieldDefinition netFieldId)
        {
            ...
            else if (fd.FieldType.IsDerivedFrom<NetworkBehaviour>() || fd.FieldType.Is<NetworkBehaviour>())
            {
                ...
                MethodReference getFunc =
                    weaverTypes.getSyncVarNetworkBehaviourReference.MakeGeneric(assembly.MainModule, fd.FieldType);
        ...
        public MethodDefinition GenerateSyncVarSetter(TypeDefinition td, FieldDefinition fd, string originalName,
            ...
            Dictionary<FieldDefinition, (FieldDefinition hookDelegateField, MethodDefinition hookMethod)>
                syncVarHookDelegates, ref bool WeavingFailed)
        {
            ...
            else if (fd.FieldType.IsDerivedFrom<NetworkBehaviour>() || fd.FieldType.Is<NetworkBehaviour>())
            {
                ...
                MethodReference getFunc =
                    weaverTypes.generatedSyncVarSetter_NetworkBehaviour_T.MakeGeneric(assembly.MainModule,
                        fd.FieldType);
                worker.Emit(OpCodes.Call, getFunc);
            }
            else
            {
                ...
                MethodReference generic =
                    weaverTypes.generatedSyncVarSetter.MakeGeneric(assembly.MainModule, fd.FieldType);
        ...
        public void ProcessSyncVar(TypeDefinition td, FieldDefinition fd,
            Dictionary<FieldDefinition, FieldDefinition> syncVarNetIds,
            Dictionary<FieldDefinition, (FieldDefinition hookDelegateField, MethodDefinition hookMethod)>
                syncVarHookDelegates, long dirtyBit, ref bool WeavingFailed)
        {
            ...
            MethodDefinition set = GenerateSyncVarSetter(td, fd, originalName, dirtyBit, netIdField,
                syncVarHookDelegates, ref WeavingFailed);
        ...
        public (List<FieldDefinition> syncVars, Dictionary<FieldDefinition, FieldDefinition> syncVarNetIds,
            Dictionary<FieldDefinition, (FieldDefinition hookDelegateField, MethodDefinition hookMethod)>
            syncVarHookDelegates) ProcessSyncVars(TypeDefinition td, ref bool WeavingFailed)
        {
            ...
            foreach (FieldDefinition fd in td.Fields)
            {
                if (fd.HasCustomAttribute<SyncVarAttribute>())
                {
                    ...
                    {
                        Log.Warning($"{fd.Name} has [SyncVar] attribute. SyncLists should not be marked with SyncVar",
                            fd);
                    }
                    else
                    {
                        ...
                        ProcessSyncVar(td, fd, syncVarNetIds, syncVarHookDelegates, 1L << dirtyBitCounter,
                            ref WeavingFailed);
                        dirtyBitCounter += 1;
