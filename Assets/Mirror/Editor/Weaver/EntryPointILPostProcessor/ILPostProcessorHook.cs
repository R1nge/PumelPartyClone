...
namespace Mirror.Weaver
{
    public class ILPostProcessorHook : ILPostProcessor
    {
        ...
        public override ILPostProcessResult Process(ICompiledAssembly compiledAssembly)
        {
            ...
            using (ILPostProcessorAssemblyResolver asmResolver =
                   new ILPostProcessorAssemblyResolver(compiledAssembly, Log))
            {
                    ...
                    using (AssemblyDefinition asmDef = AssemblyDefinition.ReadAssembly(stream, readerParameters))
                    {
                        ...
                        if (weaver.Weave(asmDef, asmResolver, out bool modified))
                        {
                            ...
                            if (modified)
                            {
                                ...
                                {
                                    asmDef.MainModule.AssemblyReferences.Remove(
                                        asmDef.MainModule.AssemblyReferences.First(r => r.Name == asmDef.Name.Name));
