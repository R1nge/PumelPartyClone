...
namespace Mirror
{
    ...
    public class PredictedRigidbody : NetworkBehaviour
    {
        ...
        [Header("Smoothing")] [Tooltip("Configure how to apply the corrected state.")]
        ...
        [Header("Debugging")] public float lineTime = 10;
        ...
        void ApplyCorrection(RigidbodyState corrected, RigidbodyState before, RigidbodyState after)
        {
            ...
            Debug.Log(
                $"Correcting {name}: {correctedCount} / {stateHistory.Count} states to final position from: {rb.position} to: {last.position}");
        ...
        void CompareState(double timestamp, RigidbodyState state)
        {
                    ...
                    out double t))
            {
                ...
                {
                    Debug.LogWarning(
                        $"Hard correcting client because the client is too far behind the server. History of size={stateHistory.Count} @ t={timestamp:F3} oldest={oldest.timestamp:F3} newest={newest.timestamp:F3}. This would cause the client to be out of sync as long as it's behind.");
                ...
                else if (newest.timestamp < state.timestamp)
                {
                    ...
                    if (Vector3.Distance(state.position, rb.position) >= correctionThreshold)
                    {
                        ...
                        Debug.Log(
                            $"Hard correction because the client is ahead of the server by {(ahead * 1000):F1}ms. History of size={stateHistory.Count} @ t={timestamp:F3} oldest={oldest.timestamp:F3} newest={newest.timestamp:F3}. This can happen when latency is near zero, and is fine unless it shows jitter.");
                        ApplyCorrection(state, state, state);
                    }
                }
        ...
        public override void OnDeserialize(NetworkReader reader, bool initialState)
        {
            ...
            CompareState(timestamp,
                new RigidbodyState(timestamp, Vector3.zero, position, rotation, Vector3.zero, velocity));
        }
    }
}