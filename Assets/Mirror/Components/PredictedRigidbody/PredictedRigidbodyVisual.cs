    ...
    public class PredictedRigidbodyVisual : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            if (distance > threshold)
            {
                ...
                transform.rotation = targetRigidbody.rotation;
                Debug.Log(
                    $"[PredictedRigidbodyVisual] Teleported because distance {distance:F2} > threshold {threshold:F2}");
                return;
            }
            ...
            transform.position =
                Vector3.MoveTowards(transform.position, targetRigidbody.position, step * Time.deltaTime);
            transform.rotation =
                Quaternion.RotateTowards(transform.rotation, targetRigidbody.rotation, step * Time.deltaTime);
        }
    }
}