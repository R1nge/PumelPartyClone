...
namespace Mirror
{
    ...
    public class NetworkTransformUnreliable : NetworkTransformBase
    {
                ...
                (syncDirection == SyncDirection.ServerToClient || IsClientWithAuthority))
            {
                ...
                if (compressRotation)
                {
                    RpcServerToClientSyncCompressRotation(
                        ...
                        syncRotation && rotationChanged
                            ? Compression.CompressQuaternion(snapshot.rotation)
                            : default(uint?),
        ...
        {
            OnClientToServerSync(position,
                rotation.HasValue ? Compression.DecompressQuaternion((uint)rotation) : target.rotation, scale);
            //For client authority, immediately pass on the client snapshot to all other
            //clients instead of waiting for server to send its snapshots.
            if (syncDirection == SyncDirection.ClientToServer)
