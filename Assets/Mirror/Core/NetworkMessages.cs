        ...
        internal static NetworkMessageDelegate WrapHandler<T, C>(Action<C, T, int> handler, bool requireAuthentication,
            ...
            => (conn, reader, channelId) =>
            {
                ...
                catch (Exception exception)
                {
                    ...
                    {
                        Debug.LogError(
                            $"Disconnecting connection: {conn} because reading a message of type {typeof(T)} caused an Exception. This can happen if the other side accidentally (or an attacker intentionally) sent invalid data. Reason: {exception}");
                        conn.Disconnect();
                        return;
                    }
                    ...
                    {
                        Debug.LogError(
                            $"Caught an Exception when reading a message from: {conn} of type {typeof(T)}. Reason: {exception}");
                        return;
                    }
                }
                finally
                ...
                catch (Exception exception)
                {
                    ...
                    {
                        Debug.LogError(
                            $"Disconnecting connection: {conn} because handling a message of type {typeof(T)} caused an Exception. This can happen if the other side accidentally (or an attacker intentionally) sent invalid data. Reason: {exception}");
                        conn.Disconnect();
                    }
                    // otherwise log it but allow the connection to keep playing
                    else
                    {
                        Debug.LogError(
                            $"Caught an Exception when handling a message from: {conn} of type {typeof(T)}. Reason: {exception}");
                    }
                }
            };
