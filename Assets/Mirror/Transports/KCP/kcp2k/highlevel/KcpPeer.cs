    ...
    public abstract class KcpPeer
    {
            ...
            if (ReceiveNextReliable(out KcpHeader header, out ArraySegment<byte> message))
            {
                ...
                switch (header)
                {
                    ...
                    case KcpHeader.Disconnect:
                    {
                        ...
                        // GetType() shows Server/ClientConn instead of just Connection.
                        OnError(ErrorCode.InvalidReceive,
                            $"{GetType()}: received invalid header {header} while Connected. Disconnecting the connection.");
                        Disconnect();
                        break;
                    }
                }
        ...
        void SendUnreliable(ArraySegment<byte> message)
        {
            ...
            if (message.Count > unreliableMax)
            {
                ...
                // GetType() shows Server/ClientConn instead of just Connection.
                Log.Error(
                    $"{GetType()}: Failed to send unreliable message of size {message.Count} because it's larger than UnreliableMaxMessageSize={unreliableMax}");
                return;
            }
