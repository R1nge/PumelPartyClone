...
namespace kcp2k
{
    public class KcpClient : KcpPeer
    {
        ...
        public void RawInput(ArraySegment<byte> segment)
        {
            ...
            {
                Log.Error(
                    $"KcpClient: received message with cookie=0, this should never happen. Server should always include the security cookie.");
            }
            ...
            {
                Log.Warning(
                    $"KcpClient: dropping message with mismatching cookie: {messageCookie} expected: {cookie}.");
                return;
            }
