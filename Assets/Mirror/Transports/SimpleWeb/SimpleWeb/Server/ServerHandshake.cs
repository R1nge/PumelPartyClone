    ...
    internal class ServerHandshake
    {
        ...
        public bool TryHandshake(Connection conn)
        {
            ...
            using (ArrayBuffer getHeader = bufferPool.Take(GetSize))
            {
                ...
                {
                    Log.Warn(
                        $"[SWT-ServerHandshake]: First bytes from client was not 'GET' for handshake, instead was {Log.BufferToString(getHeader.array, 0, GetSize)}");
                    return false;
                }
            }
        ...
        {
            Log.Verbose(
                $"[SWT-ServerHandshake]: Handshake Hashing {Encoding.ASCII.GetString(keyBuffer, 0, MergedKeyLength)}");
        ...
        static void CreateResponse(byte[] keyHash, byte[] responseBuffer)
        {
            ...
            Log.Verbose(
                $"[SWT-ServerHandshake]: Handshake Response length {message.Length}, IsExpected {message.Length == ResponseLength}");
