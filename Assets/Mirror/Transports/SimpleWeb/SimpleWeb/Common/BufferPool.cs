    ...
    public class BufferPool
    {
        ...
        void Validate()
        {
            if (buckets[0].arraySize != smallest)
                Log.Error(
                    $"[SWT-BufferPool]: BufferPool Failed to create bucket for smallest. bucket:{buckets[0].arraySize} smallest:{smallest}");
            ...
            if (largestBucket != largest && largestBucket != largest + 1)
                Log.Error(
                    $"[SWT-BufferPool]: BufferPool Failed to create bucket for largest. bucket:{largestBucket} smallest:{largest}");
        }
