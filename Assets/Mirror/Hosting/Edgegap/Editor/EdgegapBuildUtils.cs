...
";
        ...
        // MIRROR CHANGE
        static async Task RunCommand_DockerVersion(Action<string> outputReciever = null,
            Action<string> errorReciever = null)
        {
#if UNITY_EDITOR_WIN
        ...
        // MIRROR CHANGE
        public static async Task RunCommand_DockerBuild(string registry, string imageRepo, string tag,
        ...
        public static async Task<(bool, string)> RunCommand_DockerPush(string registry, string imageRepo, string tag,
            Action<string> onStatusUpdate)
        {
            ...
            await RunCommand("docker.exe", $"push {registry}/{imageRepo}:{tag}", onStatusUpdate,
                (msg) => error += msg + "\n");
#elif UNITY_EDITOR_OSX
            await RunCommand("/bin/bash", $"-c \"docker push {registry}/{imageRepo}:{tag}\"", onStatusUpdate, (msg) => error
 += msg + "\n");
#elif UNITY_EDITOR_LINUX
            await RunCommand("/bin/bash", $"-c \"docker push {registry}/{imageRepo}:{tag}\"", onStatusUpdate, (msg) => error
 += msg + "\n");
        ...
        static async Task RunCommand(string command, string arguments, Action<string> outputReciever = null,
            Action<string> errorReciever = null)
        {
            ...
            Action<string> outputReciever = null, Action<string> errorReciever = null)
        {
            ...
            try
            {
                ...
                await RunCommand("cmd.exe",
                    $"/c docker login -u \"{repoUsername}\" --password \"{repoPasswordToken}\" \"{registryUrl}\"",
                    outputReciever, errorReciever);
#elif UNITY_EDITOR_OSX
        ...
        {
            string error = null;
            await RunCommand_DockerLogin(registryUrl, repoUsername, repoPasswordToken, onStatusUpdate,
                msg => error = msg); // MIRROR CHANGE
