...
namespace Mirror.Hosting.Edgegap.Editor
{
    ...
    public class EdgegapWindowV2 : EditorWindow
    {
        ...
        public static void ShowEdgegapToolWindow()
        {
            ...
            window.titleContent =
                new GUIContent(
                    "Edgegap Hosting"); // MIRROR CHANGE: 'Edgegap Server Management' is too long for the tab space
        ...
        private async Task syncFormWithObjectDynamicAsync()
        {
            ...
            {
                if (IsLogLevelDebug)
                    Debug.Log("syncFormWithObjectDynamicAsync: Found apiToken && appName; " +
                              "calling GetAppAsync =>");
                try
                {
                    await GetAppAsync();
        ...
        private async Task createAppAsync()
        {
            ...
            CreateAppRequest createAppRequest =
                new CreateAppRequest( // MIRROR CHANGE: 'new()' not supported in Unity 2020
                    _appNameInput.value,
                    isActive: true,
        ...
        private async Task createDeploymentStartServerAsync()
        {
            ...
            try
            {
                ...
                CreateDeploymentRequest createDeploymentReq =
                    new CreateDeploymentRequest( // MIRROR CHANGE: 'new()' not supported in Unity 2020
                        _appNameInput.value,
        ...
        private void onCreateDeploymentOrRefreshSuccess(GetDeploymentStatusResult getDeploymentStatusResult)
        {
            ...
            KeyValuePair<string, DeploymentPortsData> portsDataKvp =
                getDeploymentStatusResult.PortsDict.FirstOrDefault();
            Assert.IsNotNull(portsDataKvp.Value,
                $"Expected ({nameof(portsDataKvp)} from `getDeploymentStatusResult.PortsDict`)");
            ...
            _deploymentsConnectionStopBtn.clickable.clickedWithEventInfo +=
                onDynamicStopServerBtnAsync; // Unsubscribes on click
            _deploymentsConnectionStopBtn.visible = true;
        ...
        private void onCreateDeploymentStartServerFail(EdgegapHttpResult<CreateDeploymentResult> result = null)
        {
            ...
            Debug.Log(
                "(!) Check your deployments here: https://app.edgegap.com/deployment-management/deployments/list");
        ...
        private async Task onDynamicStopServerAsync()
        {
            ...
            try
            {
                ...
                TimeSpan pollIntervalSecs =
                    TimeSpan.FromSeconds(EdgegapWindowMetadata.DEPLOYMENT_STOP_STATUS_POLL_SECONDS);
            ...
            _deploymentsStatusLabel.text =
                ""; // Overrides any previous errs, in case we attempted to created a new deployment while deleting
        ...
        private async Task buildAndPushServerAsync()
        {
            ...
            try
            {
                ...
                if (!EdgegapWindowMetadata.SKIP_DOCKER_IMAGE_BUILD_WHEN_PUSHING)
                {
                    ...
                    await EdgegapBuildUtils.RunCommand_DockerBuild(registry, imageName, tag,
                        status => ShowBuildWorkInProgress("Building Docker Image", status));
                }
                else
                ...
                bool isContainerLoginSuccess = await EdgegapBuildUtils.LoginContainerRegistry(
                    ...
                    status => ShowBuildWorkInProgress("Logging into container registry.",
                        status)); // MIRROR CHANGE: DETAILED LOGGING
                ...
                // push docker image
                (bool isPushSuccess, string error) = await EdgegapBuildUtils.RunCommand_DockerPush(registry, imageName,
                    tag, status => ShowBuildWorkInProgress("Uploading Docker Image (this may take a while)", status));
                if (!isPushSuccess)
                {
                ...
                UpdateAppVersionRequest updateAppVerReq =
                    new UpdateAppVersionRequest(_appNameInput
                            .value) // MIRROR CHANGE: 'new()' not supported in Unity 2020
                        {
                ...
                EdgegapHttpResult<UpsertAppVersionResult> updateAppVersionResult =
                    await appApi.UpsertAppVersion(updateAppVerReq);

                if (updateAppVersionResult.HasErr)
                {
                    onBuildPushError(
                        $"Unable to update docker tag/version:\n{updateAppVersionResult.Error.ErrorMessage}");
                    return;
                }
...
#pragma warning restore CS0414 // END MIRROR CHANGE
        ...
        private static readonly Regex
            _appNameAllowedCharsRegex =
                new Regex(@"^[a-zA-Z0-9_\-+\.]*$"); // MIRROR CHANGE: 'new()' not supported in Unity 2020
...
#pragma warning restore CS1998 // END MIRROR CHANGE
        ...
        private void setVisualElementsToFields()
        {
            ...
            _containerRegistryFoldout =
                rootVisualElement.Q<Foldout>(EdgegapWindowMetadata.CONTAINER_REGISTRY_FOLDOUT_ID);
            _containerNewTagVersionInput =
                rootVisualElement.Q<TextField>(EdgegapWindowMetadata.CONTAINER_NEW_TAG_VERSION_TXT_ID);
            _containerPortNumInput =
                rootVisualElement.Q<TextField>(EdgegapWindowMetadata.CONTAINER_REGISTRY_PORT_NUM_ID);
            ...
            // END MIRROR CHANGE
            _containerUseCustomRegistryToggle =
                rootVisualElement.Q<Toggle>(EdgegapWindowMetadata.CONTAINER_USE_CUSTOM_REGISTRY_TOGGLE_ID);
            _containerCustomRegistryWrapper =
                rootVisualElement.Q<VisualElement>(EdgegapWindowMetadata.CONTAINER_CUSTOM_REGISTRY_WRAPPER_ID);
            _containerRegistryUrlInput =
                rootVisualElement.Q<TextField>(EdgegapWindowMetadata.CONTAINER_REGISTRY_URL_TXT_ID);
            _containerImageRepositoryInput =
                rootVisualElement.Q<TextField>(EdgegapWindowMetadata.CONTAINER_IMAGE_REPOSITORY_URL_TXT_ID);
            ...
            _containerBuildAndPushServerBtn =
                rootVisualElement.Q<Button>(EdgegapWindowMetadata.CONTAINER_BUILD_AND_PUSH_BTN_ID);
            _containerBuildAndPushResultLabel =
                rootVisualElement.Q<Label>(EdgegapWindowMetadata.CONTAINER_BUILD_AND_PUSH_RESULT_LABEL_ID);
            ...
            _deploymentsServerDataContainer =
                rootVisualElement.Q<VisualElement>(EdgegapWindowMetadata.DEPLOYMENTS_CONTAINER_ID); // Dynamic
            _deploymentConnectionCopyUrlBtn =
                rootVisualElement.Q<Button>(EdgegapWindowMetadata.DEPLOYMENTS_CONNECTION_COPY_URL_BTN_ID);
            _deploymentsConnectionUrlReadonlyInput =
                rootVisualElement.Q<TextField>(EdgegapWindowMetadata.DEPLOYMENTS_CONNECTION_URL_READONLY_TXT_ID);
            _deploymentsConnectionStatusLabel =
                rootVisualElement.Q<Label>(EdgegapWindowMetadata.DEPLOYMENTS_CONNECTION_STATUS_LABEL_ID);
            _deploymentsConnectionStopBtn =
                rootVisualElement.Q<Button>(EdgegapWindowMetadata.DEPLOYMENTS_CONNECTION_SERVER_ACTION_STOP_BTN_ID);
            ...
            _footerNeedMoreGameServersBtn =
                rootVisualElement.Q<Button>(EdgegapWindowMetadata.FOOTER_NEED_MORE_GAME_SERVERS_BTN_ID);
        ...
        private void setDeploymentBtnsFromCache()
        {
            ...
            if (isDeployed)
                _deploymentsConnectionStopBtn.clickable.clickedWithEventInfo +=
                    onDynamicStopServerBtnAsync; // Unsub'd from within
        }
        ...
        private async void onContainerBuildAndPushServerBtnClickAsync()
        {
            ...
            try
            {
                await buildAndPushServerAsync();
            }
            finally
            {
                _containerBuildAndPushServerBtn.SetEnabled(checkHasAppName());
                _containerBuildAndPushResultLabel.visible = _containerBuildAndPushResultLabel.text !=
                                                            EdgegapWindowMetadata.PROCESSING_RICH_STR;
            }
        }
        ...
        private EdgegapDeploymentsApi getDeployApi() =>
            new EdgegapDeploymentsApi( // MIRROR CHANGE: 'new()' not supported in Unity 2020
        ...
        private EdgegapWizardApi getWizardApi() =>
            new EdgegapWizardApi( // MIRROR CHANGE: 'new()' not supported in Unity 2020
                EdgegapWindowMetadata.API_ENVIRONMENT,
                _apiTokenInput.value.Trim(),
                EdgegapWindowMetadata.LOG_LEVEL);
        ...
        private void loadPersistentDataFromEditorPrefs()
        {
            ...
            // DeploymentConnectionStatus
            if (string.IsNullOrEmpty(_deploymentsConnectionStatusLabel.text) ||
                _deploymentsConnectionStatusLabel.text == "Unknown")
