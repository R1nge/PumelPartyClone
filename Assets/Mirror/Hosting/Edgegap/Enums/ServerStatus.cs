...
namespace Mirror.Hosting.Edgegap.Enums
{
    ...
    public static class ServerStatusExtensions
    {
        private static string GetServerStatusLabel(this Status serverStatusResponse) =>
            char.ToUpper(serverStatusResponse.CurrentStatus[7]) +
            serverStatusResponse.CurrentStatus.Substring(8).ToLower();
        ...
        public static ServerStatus GetServerStatus(this Status serverStatusResponse)
        {
            ...
            {
                serverStatus =
                    (ServerStatus)Enum.Parse(typeof(ServerStatus), serverStatusResponse.GetServerStatusLabel());
            }
            catch (Exception)
            {
                Debug.LogError(
                    $"Got unexpected server status: {serverStatusResponse.CurrentStatus}. Considering the deployment to be terminated.");
